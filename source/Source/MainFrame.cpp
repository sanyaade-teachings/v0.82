...
//    dialog0.ShowModal(); //##Debug
    ...
    bubble.setToolsPath(bubble.getComponentsRepositoryPath() + wxString("/WinAVR/v20090313/bin")); //##Un-hardcode...

    //##Horrible, súper temporal:
    wxFileName _tempPath(bubble.getComponentsRepositoryPath() + wxString("/_Workspaces/_Temp")); //##Un-hardcode...
    ...
    bubble.setTempPath(bubble.getComponentsRepositoryPath() + wxString("/_Workspaces/_Temp")); //##Un-hardcode...
...
//##: Ojo, que estas líneas estaban produciendo errores:
    ...
    //##Horrible, súper temporal:
    wxFileName aux(tempComponentName);
    //bubble.setComponentPath(bubble.getComponentsRepositoryPath() + wxString("/_Workspaces"));
    bubble.setComponentPath(bubble.getTempPath());
    bubble.setOutputPath(bubble.getComponentPath() + wxString("/") + aux.GetName() + wxString("_Files/Output"));
}
...
void MainFrame::onMenuFileAdd(wxCommandEvent& evt)
{
    ...
    //##Falta obtener el último dir que el usuario quiere usar, y por defecto el _Workspaces, etc..:
...
{
    //##Si if this code is portable, and if this will show some kind or error message in case the command fails:
    wxString folderPath = bubble.getComponentsRepositoryPath() + wxString("/_Workspaces/_Examples");
